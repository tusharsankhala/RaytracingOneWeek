...
template <typename T>
inline vec3<T> cross(const vec3<T>& vec1, const vec3<T>& vec2)
{
...
template <typename T>
inline static vec3<T> refract(const vec3<T>& uv, const vec3<T>& n, double etai_over_etat)
{
	auto cos_theta = dot(-uv, n);
	vec3<T> r_out_parallel = etai_over_etat * (uv + cos_theta * n);
	vec3<T> r_out_perp = -sqrt(1.0 - r_out_parallel.magnitude_squared()) * n;
